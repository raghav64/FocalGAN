# -*- coding: utf-8 -*-
"""FocalClassifier_cifar10

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1AKbU_33T4GoY9eBxwmvHt9vSPmmpjrv_
"""
...
def get_cimbalanced_train_data(drop_list, drop_percentage_list, x_train, y_train):
...
def focal_loss_fnc(drop_mask, y_pred, y_label, inverse_class_weights, gamma=2.):
...
#     alpha = float(alpha)
    ...
    y_ground = tf.argmax(y_label, 1)
    
    
    alpha = tf.matmul(y_label, inverse_class_weights, name = 'alpha')
    
    model_out = tf.add(y_pred, epsilon, name = 'model_out')
    ce = tf.multiply(y_true, -tf.log(model_out), name = 'ce')
    modulator = tf.pow(tf.subtract(1., model_out), gamma, name = 'modulator')
    weight = tf.multiply(y_true, modulator, name = 'weight')
    fl = tf.multiply(alpha, tf.multiply(weight, ce), name = 'fl')
    
    reduced_fl = tf.reduce_max(fl, axis = 1)
    
    imbalanced_loss = tf.multiply(drop_mask, reduced_fl)
    imb_loss = tf.reduce_mean(imbalanced_loss)
    
    total_loss = tf.reduce_mean(reduced_fl)
    
    return total_loss, imb_loss, total_loss-imb_loss
#imb loss is loss due to classes in drop list class

def focal_classifier(x, dropout_rate = 0.):
...
#         w_init = tf.keras.initializers.glorot_normal()
        # Layer 1
        conv1 = tf.layers.conv2d(x, 16, kernel_size = [2,2],
                                         strides = [1,1], padding = 'same', name = 'conv1', activation = 'relu')
        print (conv1.shape)
        # ?*32*32*16
        
        # Layer 2
        conv2 = tf.layers.conv2d(conv1, 32, kernel_size = [3,3],
                                 strides = [1,1], padding = 'same', name = 'conv2', activation = 'relu')
        print (conv2.shape)
        # ?*32*32*32
        max_pool1 = tf.nn.max_pool(conv2, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME', name = 'pool1')
        print (max_pool1.shape)
        # ?*16*16*32
        
        # Layer 3
        conv3 = tf.layers.conv2d(max_pool1, 64, kernel_size = [4,4],
                                         strides = [1,1], padding = 'same', name = 'conv3', activation = 'relu')
        print (conv3.shape)
        # 16*16*64
        max_pool2 = tf.nn.max_pool(conv3, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME', name = 'pool2')
        print (max_pool2.shape)
        # ?*8*8*64
        
        # Layer 4
        conv4 = tf.layers.conv2d(max_pool2, 128, kernel_size = [4,4],
                                         strides = [1,1], padding = 'same', name = 'conv4', activation = 'relu')
        print (conv4.shape)
        # ?*8*8*128
        max_pool3 = tf.nn.max_pool(conv4, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME', name = 'pool3')
        print (max_pool3.shape)
        # ?*4*4*128
        
        dropout1 = tf.nn.dropout(max_pool3, dropout_rate, name = 'dropout1')
        
        flatten = tf.reshape(dropout1, [-1, 4*4*128], name = 'flatten')
        dropout2 = tf.nn.dropout(flatten, dropout_rate, name = 'dropout2')
      
        layer = tf.layers.dense(dropout2, 512, name = 'layer')
        dropout3 = tf.nn.dropout(layer, dropout_rate, name = 'dropout3')
        
        logits = tf.layers.dense(dropout3, 10, name = 'logits', bias_initializer = tf.constant_initializer(value = bias_value))
        print (logits.shape)
        #?*10
...
def loss_fnc(mask, x, y_label, inverse_class_weights, dropout_rate):
    logits, out = focal_classifier(x, dropout_rate)
    with tf.variable_scope('Loss_fnc', reuse = tf.AUTO_REUSE):
        class_loss, imb_loss, bal_loss = focal_loss_fnc(mask, out, y_label, inverse_class_weights)

    return class_loss, out, imb_loss, bal_loss
...
#     mask = tf.Variable(name = 'mask', initial_value = tf.ones(shape = (batch_size,) ) )
    ...
    # Building the graph
    class_loss, out, imb_loss, bal_loss = loss_fnc(drop_mask, x, y_label, inverse_class_weights, dropout_rate)
...
#     saver = tf.train.Saver(max_to_keep = 2)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            correct_pred = tf.equal(y__pred, y__test)
