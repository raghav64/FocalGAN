# -*- coding: utf-8 -*-
"""FocalClassifier_MNIST

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1HWSCvlSGgKblRwwEpQvSgUaq4XmAwIhq
"""
...
#     out, idx = tf.setdiff1d(y_ground, drop_list)
#     #idx contains indices of y_ground which are not in drop_list i.e balanced data 
    
    
# #     print (type(tf.Variable(mask)))
# #     print (mask)
#     drop_mask = tf.scatter_update(tf.Variable(initial_value = mask), idx, tf.zeros_like(idx, tf.float32))
...
# drop_mask = tf.scatter_nd(indices, tf.ones(dtype = tf.int32, shape = (10,)), shape)
# print (x_data,type(x_data))
# with tf.Session() as sess:
#     sess.run(tf.global_variables_initializer())
# #     print (sess.run(out, feed_dict = {x:x_data}))
#     print(sess.run(mask))
#     indice = idx.eval(feed_dict = {x:x_data})
#     am = indice.shape[0]
#     indice = np.reshape(indice, (am, 1))
#     print ('indice shape: ' + str(indice.shape))
#     print (x_data.shape)
# #     print (shap.eval())
#     print (drop_mask.eval(feed_dict = {indices: indice, shape:x_data.shape}))
#     sess.close()
# # print (tr[1])
...
#         w_init = tf.keras.initializers.glorot_normal()
        # Layer 1
        conv1 = tf.layers.conv2d(x, 32, kernel_size = [3,3],
...
#         dropout3 = tf.nn.dropout(flatten, dropout_rate, name = 'dropout3')
      
#         layer = tf.layers.dense(dropout3, 896, name = 'layer')
        
        logits = tf.layers.dense(flatten, 10, name = 'logits', bias_initializer = tf.constant_initializer(value = bias_value))
        print (logits.shape)
